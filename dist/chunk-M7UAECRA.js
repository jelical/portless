...
function createLoopbackConnection(port) {
  return net.connect({
    host: "localhost",
    port,
...
function tailscaleAuthority(tailscaleUrl) {
  if (!tailscaleUrl) return void 0;
  try {
...
function createProxyServer(options) {
  ...
  const handleUpgrade = (req, socket, head) => {
    ...
    if (hops >= MAX_PROXY_HOPS) {
      ...
      onError(
        `WebSocket loop detected for ${host}: request has passed through portless ${hops} times. Set changeOrigin: true in your proxy config.`
      );
      socket.end(
...
`;
  ...
  const handleExtendedConnect = (req, res) => {
    const compatReq = req;
    req.stream.on("error", () => {
    });
    res.setHeader(PORTLESS_HEADER, "1");
...
`);
    ...
    proxyReq.on("upgrade", (proxyRes, proxySocket, proxyHead) => {
      const expectedAccept = crypto.createHash("sha1").update(wsKey + WS_GUID).digest("base64");
  ...
  if (tls) {
    ...
    h2Server.on(
      "connect",
      (req, resOrSocket) => {
        if (resOrSocket instanceof net2.Socket) {
          resOrSocket.destroy();
          return;
        }
        handleExtendedConnect(req, resOrSocket);
      }
    );
    ...
    const wrapper = net2.createServer((socket) => {
      socket.on("error", () => {
        socket.destroy();
      });
...
var RouteStore = class _RouteStore {
   ...
   */
  saveRoutes(routes) {
    const tmpPath = path3.join(this.dir, `routes.json.tmp-${process.pid}`);
    try {
      fs3.writeFileSync(tmpPath, JSON.stringify(routes, null, 2), { mode: FILE_MODE });
